import { Container, createStyles, Paper, ScrollArea, Skeleton, Table, Text } from '@mantine/core'
...
const fetchBrowserData = async (url: string, email: string) => {
  const session_data = await fetcher(`${BASE_URL.SERVER}/api/users/getBrowsersByEmail`, {
    method: 'POST',
...
const useBrowserData = (email: string) => {
  const { data, mutate, error, isValidating } = useSWR(['browser', email], fetchBrowserData, {
...
const fetchSessionData = async (url: string, email: string) => {
  const session_data = await fetcher(`${BASE_URL.SERVER}/api/session/getAvailableSessions`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      email: email,
...
const useSessionData = (email: string) => {
  const { data, mutate, error, isValidating } = useSWR(['session', email], fetchSessionData, {
    revalidateOnFocus: false,
  })
  return {
    data: data,
...
export default function BrowserControl() {
  ...
  const handleSetSessionToBrowser = async (session_id, browser_id) => {
    setIsHandling(true)
    const data = await fetcher(`${BASE_URL.SERVER}/api/session/setSessiontoBrowser`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
  ...
  const handleOnDragEnd = (result) => {
    ...
    if (result.destination.droppableId !== 'sessions') openModal(result.draggableId, result.destination.droppableId)
  }
  useEffect(() => {
          ...
          <DragDropContext onDragEnd={handleOnDragEnd}>
            ...
            <Paper shadow="xl" radius="md">
              <ScrollArea
                ...
                <Skeleton visible={isLoadingB || isHandling} style={{ height: 200 }}>
                          ...
                          <Droppable key={index} droppableId={index.toString()}>
                            {(provided) => (
                              <tr {...provided.droppableProps} ref={provided.innerRef}>
                                <td>{index}</td>
                                <td>{browser.name === '' ? 'No Session' : browser.name}</td>
                          ...
                          <tbody ref={provided.innerRef}>
                            {session_data &&
                              session_data.map((session, index) => (
                                <Draggable key={session.name} draggableId={session._id} index={index}>
                                  {(provided, snapshot) => (
                                    <>
                                      <tr
                                        ...
                                        <td>
                                          <div {...provided.dragHandleProps}>
                                            <ToolTipButton description="Drag" compact>
