...
import { IconActivity, IconListDetails, IconPlus, IconPoint, IconSearch } from '@tabler/icons'
...
const SessionControl = () => {
  ...
  const { data, isLoading, isError, mutate } = useControlSession(session.user.email)
    ...
    <Container mt="xl" className={classes.container}>
      ...
      <Modal
        ...
        <form onSubmit={form.onSubmit((values) => handleCreateNewSession(values))}>
          <SimpleGrid cols={1}>
            <TextInput placeholder="Name" label="Session Name" withAsterisk {...form.getInputProps('name')} />
            <Textarea
                ...
                {data.map((session, index) => {
                  ...
                  return (
                    <tr key={index}>
                      ...
                      <td>{session.name}</td>
                      <td>{session.users.length == 0 ? 'No Available Users' : session.users.length + ' Users'}</td>
                      <td>
                        <Group>
                          <IconPoint color={session.isActive ? 'green' : 'red'} size={24} />
                      ...
                      <td width={200}>
                        <div
                          ...
                          <ToolTipButton
                            ...
                            color={session.isActive ? 'red' : 'green'}
                            variant="subtle">
                            <IconActivity />
                          </ToolTipButton>
                          <ToolTipButton
