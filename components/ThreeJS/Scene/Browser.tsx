...
function Browser(props) {
  ...
  const unloadBrowser = useCallback(async () => {
    if (userBrowser[props.bid].url == 'none' || userBrowser[props.bid].url == 'No Session') return
    const response = await fetcher(`${BASE_URL.SERVER}/api/session/removeParticipant`, {
      method: 'POST',
  ...
  const loadBrowser = useCallback(async () => {
    ...
    // let embedURL
    if (embedURL == null || embedURL == 'none' || embedURL == 'No Session') return
    try {
  ...
  const handleOnSessionTerminated = useCallback(() => {
    showNotification({
      title: `Session Ended`,
      message: `${userBrowser[props.bid].name} is Terminated! Contact the creator to activate it`,
      color: 'yellow',
      autoClose: false,
    })
  }, [])
  ...
  return (
    <animated.group {...props}>
      <mesh
        ...
        ref={meshobject}
        material={
          userBrowser && userBrowser[props.bid].url != 'none' && userBrowser[props.bid].url != 'No Session'
            ? material
            : defMaterial
        }
        geometry={geometry}
